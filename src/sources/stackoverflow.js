...
// job's id comes after the /jobs/ in the url path
...
class StackOverflow {
  ...
  _extractResults = async (page) => {
    ...
    for (const link of jobLinks.slice(-2)) {
      try {
        ...
        // logger.info(err.msg);
        // return new ScrapeError(err.msg, _, 'INSIDE StackOverflow Scraper: extractResults function ', true);
        console.log('_extractResults  ERROR::: \n ', err)
      }
  ...
  _getLinks = async (page) => {
    ...
    const jobLinks = await page.$$eval(selector, all => all
      .map(element => element.querySelector('[href]')['href']))
  ...
  _getJobInfo = async (page) => await page.evaluate(url => {
    ...
    const salary = salaryElement.offsetParent.className === 'container' ? salaryElement.textContent : null;
